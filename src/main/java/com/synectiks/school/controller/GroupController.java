...
public class GroupController {
    ...
    @PostMapping("/groups/{groupName}/users/batch")
    ...
    @DeleteMapping("/groups/{groupName}/users/{userId}")
    ...
    //New Endpoints to expose user data
    @GetMapping("/users")
    public ResponseEntity<?> getAllUsers(@PathVariable String schoolId) {
        try {
            List<Map<String, Object>> users = groupService.getAllUsers(schoolId);
    ...
    public ResponseEntity<?> getUsersByRole(@PathVariable String schoolId, @PathVariable String role) {
        try {
            List<Map<String, Object>> users = groupService.getUsersByRole(schoolId, role);
            return ResponseEntity.ok(users);
        } catch (Exception e) {
            Map<String, String> errorResponse = new HashMap<>();
            errorResponse.put("error", "Error fetching users by role: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).body(errorResponse);
        }
    }
}